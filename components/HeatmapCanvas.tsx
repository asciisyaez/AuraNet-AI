...
import { WallSegment, computePixelSignalStrength, mapWallsToSegments } from '../utils/heatmap';
...
const HeatmapCanvas: React.FC<HeatmapCanvasProps> = ({ aps, walls, width, height, show }) => {
  ...
  const wallSegments: WallSegment[] = useMemo(() => mapWallsToSegments(walls), [walls]);
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const { bestSignal, clamped } = computePixelSignalStrength(
          aps,
          wallSegments,
          x,
          y,
